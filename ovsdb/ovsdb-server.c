    ...
    *exiting = false;
    ...
    while (!*exiting) {
        ...
        if (last_role != *is_backup) {
            ovsdb_jsonrpc_server_reconnect(jsonrpc, *is_backup);
        }
...
main(int argc, char *argv[])
{
    ...
    /* If running as a backup server, start the jsonrpc server as
     * read-only server. */
              ...
              char **sync_from, char **sync_exclude, bool *is_backup)
{
    ...
    int argc = *argcp;
    char **argv = *argvp;
    bool has_sync_from = false, has_no_sync = false;
    ...
    *sync_exclude = NULL;
    ...
    free(short_options);
    *is_backup = has_sync_from && !has_no_sync;
