...
/* Session set. */
static int ovsdb_jsonrpc_sessions_count(const struct ovs_list *,
...
static struct ovsdb_jsonrpc_session *ovsdb_jsonrpc_session_create(
    struct ovsdb_jsonrpc_server *, struct jsonrpc_session *js,
    struct ovsdb_jsonrpc_remote *);
 ...
 * will not destroyed. */
#define N_SESSIONS_THRESHHOLD  (2)
...
static void
ovsdb_jsonrpc_server_del_remote(struct ovsdb_jsonrpc_server *server,
                                struct shash_node *node)
{
    struct ovsdb_jsonrpc_remote *remote = node->data;

    server = ovsdb_jsonrpc_remote_get_server(remote);
    ovsdb_jsonrpc_server_close_sessions(server, remote);
                                    ...
                                    struct ovsdb_jsonrpc_remote *remote)
{
    ...
    if (svr->n_active_threads) {
        ...
        stop = xmalloc(sizeof *stop);
        go = xmalloc(sizeof *go);
...
ovsdb_jsonrpc_server_use_threads(struct ovsdb_jsonrpc_server *svr) {
    if (svr->n_active_threads != svr->n_max_threads) {
        ...
        /* Check if more threads should be used.   */
...
ovsdb_jsonrpc_server_create_session(struct ovsdb_jsonrpc_server *svr,
                                    ...
                                    struct ovsdb_jsonrpc_remote *remote)
{
    uint8_t dscp = ovsdb_jsonrpc_remote_dscp(remote);
    ...
    if (!ovsdb_jsonrpc_server_use_threads(svr)) {
        ...
        msg = ipc_create_add_session_command(stream, svr, remote, dscp);
...
ovsdb_jsonrpc_session_create__(struct ovsdb_jsonrpc_remote *remote,
                               struct ovsdb_jsonrpc_server *server,
                               struct jsonrpc_session *js)
{
                                    ...
                                    struct jsonrpc_session *js)
{
    ...
    ovs_list_push_back(&thread->all_sessions, &s->node);
...
static int
ovsdb_jsonrpc_session_run(struct ovsdb_jsonrpc_session *s, struct seq *doorbell)
    OVS_EXCLUDED(mutex)
{
...
sessions_thread_command_run(struct sessions_thread *thread)
{
    ...
    if (new_cmd_seq != thread->last_cmd_seq) {
        ovs_mutex_lock(&thread->cmd_queue_mutex);
        while (!ovs_list_is_empty(&thread->cmd_queue)) {
             struct ipc_msg *msg;
             struct ovs_list *node;

             node = ovs_list_pop_front(&thread->cmd_queue);
...
{
    ovs_mutex_lock(&thread->cmd_queue_mutex);
    ovs_list_push_back(&thread->cmd_queue, &msg->list);
    ovs_mutex_unlock(&thread->cmd_queue_mutex);
...
/* Return the first session held in 'server' for the given 'remote'. */
struct ovsdb_jsonrpc_session *
ovsdb_jsonrpc_server_first_session(struct ovsdb_jsonrpc_server *svr,
                                   struct ovsdb_jsonrpc_remote *remote)
{
    const struct ovs_list *sessions = &svr->all_sessions;
    struct ovsdb_jsonrpc_session *s;
