...
static void
ofp_print_meter(struct ds *s, uint32_t meter_id, char sep)
{
    if (meter_id <= OFPM13_MAX) {
        ds_put_format(s, "meter%c%"PRIu32, sep, meter_id);
    } else {
        const char *name;
        switch (meter_id) {
        ...
        ds_put_format(s, "meter%c%s", sep, name);
    }
}
